...
function saveAndPrint(datos) {
    ...
    for (let i = 0; i < datos.length; i++) {
        if (datos[i].type === "multiple") {
            ...
            container.innerHTML += `
            <div class="card mt-5 align-items-center" aria-label="Basic radio toggle button group" id="multiple-answer-questions-container">

                    <h5>${questions[i]}</h5>

                <div class="mt-2">
                    <input hidden="true" type="radio" class="btn-check" name="${i}" id="btn-${i}" autocomplete="off">
            ...
            correctAnswers.push([datos[i].correct_answer]);
            container.innerHTML += `
                <div class="card align-items-center" aria-label="Basic radio toggle button group" id="boolean-questions-container">
